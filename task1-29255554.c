...
// Description: Implementation of First Come First Served Algorithm
...
#define MAX_PROC 100
#define NAME_LEN 11
#define OUTPUT_FILE "results-1.txt"
...
void run(scheduler_t *scheduler, FILE *fp) {
    ...
    while (1) {
        ...
        if (scheduler->running_proc == -1 && scheduler->proc_num > 0) {
            switch_process(scheduler, results_fp, -1, 0); // run process that came earliest
        }
